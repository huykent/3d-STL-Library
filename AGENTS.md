...
| Layer | Technology |
|---|---|
| Backend | Python 3.12 + FastAPI (async, streaming proxy) |
| Telegram | Telethon (Userbot — personal account) |
| Queue | arq (asyncio-native Redis queue) |
| 3D Analysis | FastMeshReader (instant binary header parsing <0.001s, 0MB RAM) |
| Large File Support | Up to 10GB+ via chunked streaming & direct cloud relay |
| AI Tagging & Studio | Ollama on Mac Mini M4 via Cloudflare Tunnel |
| Database | PostgreSQL (SQLAlchemy async + Alembic, BigInteger file sizes) |
| Frontend | Next.js 14 (App Router) + TailwindCSS |
| 3D Viewer | Three.js STLLoader (for single STL) + HD Archive Gallery (for ZIP/RAR) |
...
1. **No permanent local file storage & Zero-Disk Relay** — STL/ZIP files are NOT stored on the VPS. Large files up to 10GB are relayed directly or streamed on demand. Only thumbnails/images are kept.

2. **Temp files MUST be cleaned up** — Worker deletes temporary directories in `try/finally` blocks unconditionally.

3. **FastMeshReader for zero-RAM 3D inspection** — Binary STL reads only 84 bytes header; ZIP/RAR archives inspect table of contents for part count and pre-supported detection. No heavy trimesh or CPU pyrender on VPS.
...
This project is indexed by GitNexus as **telebot** (1398 symbols, 3008 relationships, 82 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
