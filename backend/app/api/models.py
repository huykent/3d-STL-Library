...
def _build_query(filters: FilterParams):
    ...
    if filters.is_presupported is not None:
        stmt = stmt.where(Model3D.is_presupported == filters.is_presupported)
    if filters.studio:
        stmt = stmt.where(Model3D.studio_name.ilike(f"%{filters.studio}%"))
...
@router.get(
    "/studios",
    summary="Get list of all distinct studio names",
)
async def list_studios(
    ...
    stmt = (
        select(Model3D.studio_name)
        .where(Model3D.studio_name.is_not(None))
        .distinct()
        .order_by(Model3D.studio_name.asc())
    )
...
async def list_models(
    ...
    filters = FilterParams(
        ...
        is_presupported=is_presupported,
        studio=studio,
...
@router.get(
    "/{model_id}/download",
    summary="Stream or download the original 3D file at maximum speed",
)
