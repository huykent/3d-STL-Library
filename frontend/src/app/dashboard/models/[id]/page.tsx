...
export default function ModelDetailPage() {
  ...
  const isArchive = ['zip', 'rar', '7z', 'tar', 'gz'].includes((model.file_extension || '').toLowerCase());
  const rawSize = model.file_size_bytes || 0;
  const sizeFormatted = rawSize >= 1024 * 1024 * 1024
            ...
            <Card className="bg-[#1c2128] border-white/10">
              ...
              <CardContent>
                <dl className="space-y-4 text-sm">
                  ...
                  {model.is_presupported && (
                    <div className="flex justify-between items-center">
                      <dt className="text-gray-400">Support Status</dt>
                      <dd>
...
function ModelViewerWrapper({ model, isArchive, onSpecsComputed }: { model: any; isArchive: boolean; onSpecsComputed?: (specs: any) => void }) {
  ...
  useEffect(() => {
    if (isArchive) return; // Do not fetch full 3D blob for zip/rar archives
  ...
  if (isArchive) {
    return (
      <div className="w-full h-full min-h-[400px] bg-gradient-to-br from-gray-900 to-black rounded-lg flex flex-col items-center justify-center border border-white/10 p-8 text-center relative overflow-hidden">
        {model.thumbnail_url && (
          <img
        ...
        <div className="relative z-10 max-w-md space-y-3">
          <div className="w-16 h-16 rounded-2xl bg-blue-500/20 border border-blue-500/40 flex items-center justify-center mx-auto text-3xl shadow-lg">
            📦
          </div>
          <h3 className="text-xl font-bold text-white tracking-tight">Gói Bộ Sưu Tập 3D (Archive)</h3>
          <p className="text-sm text-gray-400">
            Tệp tin này chứa {model.part_count || 1} file/part 3D bên trong. Xem trước qua Album ảnh demo bên dưới hoặc tải file để giải nén in 3D.
          </p>
          {model.is_presupported && (
            <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-semibold">
              🟢 Đã có file Pre-Supported đi kèm
            </div>
          )}
        </div>
      </div>
