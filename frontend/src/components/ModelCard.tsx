...
export interface Model3D {
  ...
  is_presupported?: boolean;
  studio_name?: string;
  telegram_message_id?: number;
  telegram_target_message_id?: number;
  vertex_count?: number;
  face_count?: number;
  part_count?: number;
  detail_level?: string;
        ...
        <Link href={`/dashboard/models/${model.id}`} className="flex flex-col h-full z-0">
          ...
          <CardContent className="p-4 flex flex-col flex-grow">
            <div className="flex items-center gap-1.5 mb-1 flex-wrap">
              {model.studio_name && (
                <Badge variant="outline" className="bg-purple-500/20 text-purple-300 border-purple-500/40 text-[10px] py-0 px-1.5">
                  🏷️ {model.studio_name}
                </Badge>
              )}
              {model.is_presupported && (
                <Badge variant="outline" className="bg-emerald-500/20 text-emerald-300 border-emerald-500/40 text-[10px] py-0 px-1.5">
                  🟢 Pre-Supported
                </Badge>
              )}
            </div>
            ...
            <p className="text-xs text-gray-500 truncate mb-3" title={model.original_filename}>
            ...
            <div className="mt-auto flex items-center justify-between pt-2 border-t border-white/5">
              ...
              <span className="text-xs font-medium text-gray-400">
                {model.file_size_bytes
                  ? model.file_size_bytes >= 1024 * 1024 * 1024
                    ? `${(model.file_size_bytes / (1024 * 1024 * 1024)).toFixed(1)} GB`
                    : `${(model.file_size_bytes / (1024 * 1024)).toFixed(0)} MB`
                  : model.face_count
                    ? `${(model.face_count / 1000).toFixed(1)}k faces`
                    : '3D Model'}
              </span>
            </div>
          </CardContent>
